#!/bin/bash
# ============================================================
#  UNIVERSAL MAP QUESTION CHECKER — full setup
...
/**
 * Three-layer fuzzy matcher — same logic as your extract script.
...
async function extractDotsFromMap(imageBase64: string, mimeType: string) {
...
async function extractStudentAnswers(imageBase64: string, mimeType: string) {
  const response = await client.messages.create({
    model: "claude-opus-4-5",
    max_tokens: 2048,
    messages: [
...
// ── Convert PDF pages to base64 images via sharp/pdfjs ───────
// NOTE: For production, use pdftoppm via child_process or a
// server-side PDF library. This stub expects the client to send
// pre-rasterized JPEG pages as base64 strings.
// See /lib/rasterizePdf.ts for the pdftoppm helper.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Step 2 — extract student answers (first page for now; extend for multi-page)
 ...
 * Requires pdftoppm to be installed:  apt install poppler-utils
 *
 * Usage:
      ...
      {response && (
        <div className="mt-8 space-y-6">
          ...
          <div className="p-6 bg-gray-50 rounded-xl flex items-center gap-8">
            <div>
              <div className="text-4xl font-bold">{response.totalMarks} <span className="text-xl font-normal text-gray-500">/ {response.maxTotal}</span></div>
              ...
              {response.results.map((r: Result) => (
                <tr key={r.number} className="border-b border-gray-100">
                  ...
                  <td className="px-3 py-2">
                    {r.studentSite
                      ? <><span className={r.siteRight ? "text-green-700" : "text-red-600"}>{r.studentSite}</span>
                          {r.studentState && <span className={`ml-1 text-xs ${r.stateRight ? "text-green-600" : "text-red-400"}`}>({r.studentState})</span>}
                        </>
                  ...
                  <td className="px-3 py-2 text-gray-600">
                    ...
                    {r.correctState && <span className="ml-1 text-xs text-gray-400">({r.correctState})</span>}
                  </td>
...
echo ""
echo "✅ All files written. Summary:"
echo "  lib/fuzzyMatch.ts          — three-layer fuzzy matcher"
echo "  lib/buildAnswerKey.ts      — auto-builds answer key from map dots + bookData"
echo "  lib/checkAnswers.ts        — compares student JSON to answer key, scores"
echo "  lib/rasterizePdf.ts        — server-side PDF → JPEG pages via pdftoppm"
echo "  app/api/check-map/route.ts — Next.js POST route wiring it all together"
echo "  app/check-map/page.tsx     — upload UI + results table"
echo ""
echo "Install deps if not already:"
echo "  npm install @anthropic-ai/sdk"
echo "  apt install poppler-utils   # for pdftoppm"
