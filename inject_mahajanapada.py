...
# Find the slug entry and replace its content
pattern = r'("' + slug + r'":\s*`)[^`]*(`)'
replacement = r'\g<1>' + new_html + r'\g<2>'

new_content = re.sub(pattern, replacement, content, flags=re.DOTALL)

if new_content == content:
    print(f"ERROR: slug '{slug}' not found or pattern did not match.")
else:
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(new_content)
    print(f"SUCCESS: '{slug}' content injected.")
