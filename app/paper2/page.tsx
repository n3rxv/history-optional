    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem 4rem' }}>
      <div style={{ marginBottom: '2.5rem' }}>
        ...
        <p style={{ color: 'var(--text2)', fontSize: '0.95rem', maxWidth: 600 }}>
          {paper2Notes.length} topics covering European penetration, Indian nationalism, World Wars and post-colonial world.
        </p>
      </div>
      ...
      {paper2Sections.map(section => (
        <div key={section} style={{ marginBottom: '2.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <span style={{ color: 'var(--text3)', fontSize: '0.75rem' }}>{paper2Notes.filter(n => n.section === section).length} topics</span>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '0.85rem' }}>
            {paper2Notes.filter(n => n.section === section).map((note) => (
              <Link key={note.slug} href={`/notes/${note.slug}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8,
                  padding: '1.1rem 1.25rem', transition: 'all 0.15s', cursor: 'pointer',
                ...
                >
                  <span style={{
                    minWidth: 28, height: 28, background: 'var(--accent-dim)', border: '1px solid var(--accent2)',
                    borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '0.7rem', fontFamily: 'var(--font-mono)', color: 'var(--accent)', fontWeight: 600, flexShrink: 0,
                  }}>{note.topic}</span>
                  <div>
                    <div style={{ color: 'var(--text)', fontWeight: 500, fontSize: '0.9rem', marginBottom: '0.25rem', lineHeight: 1.3 }}>{note.title}</div>
