...
const events: Event[] = [
  ...
  { year: 1789, yearLabel: "1789 CE", title: "French Revolution", description: "Fall of Bastille; Declaration of Rights of Man and Citizen; end of Ancien Regime; ideas of liberty, equality, fraternity spread globally.", category: "World History", paper: 2, section: "World History" },
  ...
  { year: 1813, yearLabel: "1813 CE", title: "Charter Act 1813", description: "EIC loses trade monopoly (except tea and China trade); Christian missionaries allowed; 1 lakh rupees for Indian education.", category: "Modern", paper: 2, section: "Modern India" },
  ...
  { year: 1930, yearLabel: "1930 CE", title: "Dandi March & Civil Disobedience", description: "Gandhi's Dandi March (March 12 to April 6); Salt Satyagraha; Civil Disobedience Movement launched; First Round Table Conference (without Congress).", category: "National Movement", paper: 2, section: "Modern India" },
  ...
  { year: 1943, yearLabel: "1943 CE", title: "Bengal Famine & Battle of Stalingrad", description: "Bengal Famine — 2 to 3 million die; Churchill blamed; Subhas Chandra Bose takes over INA in Southeast Asia. Battle of Stalingrad ends — Soviet turning point.", category: "National Movement", paper: 2, section: "Modern India" },
...
// Categories collapse onto the four categorical roles, which are the four
// syllabus sections. The sub-period is already named in the pill beside the
// year, so the colour carries the section rather than repeating the label.
const CATEGORY_COLORS: Record<string, string> = {
  'Ancient India':     'var(--ancient-text)',
  'Mauryan':           'var(--ancient-text)',
  'Gupta':             'var(--ancient-text)',
  'Medieval':          'var(--medieval-text)',
  'Sultanate':         'var(--medieval-text)',
  'Vijayanagara':      'var(--medieval-text)',
  'Mughal':            'var(--medieval-text)',
  'Modern':            'var(--modern-text)',
  'National Movement': 'var(--modern-text)',
  'World History':     'var(--world-text)',
};
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '2rem' }}>
        {CATEGORIES.filter(c => filterCategory === 'all' || c === filterCategory).map(cat => (
          <span key={cat} style={{
            ...
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: CATEGORY_COLORS[cat] || 'var(--accent-text)', flexShrink: 0 }} />
            {cat}
          </span>
        ))}
      </div>
          ...
          {filtered.map((event, i) => (
            <div key={i} className="timeline-row" style={{
              ...
              <div className="timeline-dot" style={{
                ...
                top: '50%', marginTop: -12,
                background: 'var(--bg)',
                border: `2px solid ${CATEGORY_COLORS[event.category] || 'var(--accent-text)'}`,
                borderRadius: '50%',
                width: 12, height: 12,
                zIndex: 2,
                cursor: 'pointer',
              ...
              onClick={() => { if (requireLogin('Sign in free to explore the UPSC History timeline.')) setSelected(selected?.title === event.title ? null : event); }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = CATEGORY_COLORS[event.category] || 'var(--accent-text)'; }}
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem', marginBottom: '0.35rem', flexWrap: 'wrap' }}>
                  <span style={{
                    ...
                    color: CATEGORY_COLORS[event.category] || 'var(--accent-text)',
                    background: `color-mix(in srgb, ${CATEGORY_COLORS[event.category]} 9%, transparent)`,
                    border: `1px solid color-mix(in srgb, ${CATEGORY_COLORS[event.category]} 70%, transparent)`,
