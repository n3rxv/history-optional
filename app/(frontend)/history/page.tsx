...
function ScoreRing({ pct, size = 48 }: { pct: number; size?: number }) {
  ...
  const color = pct >= 70 ? 'var(--green)' : pct >= 50 ? 'var(--warning-text)' : 'var(--danger-text)';
  return (
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem 5rem' }}>
      ...
      {mounted && entries.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.85rem', marginBottom: '2rem' }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '1rem 1.25rem' }}>
            ...
            <div style={{ color: avgScore >= 70 ? 'var(--green)' : avgScore >= 50 ? 'var(--warning-text)' : 'var(--danger-text)', fontSize: '1.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{Math.round(avgScore)}%</div>
          </div>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '1rem 1.25rem' }}>
            <div style={{ color: 'var(--text3)', fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.4rem' }}>Trend</div>
            <div style={{ color: trend > 0 ? 'var(--green)' : trend < 0 ? 'var(--danger-text)' : 'var(--text3)', fontSize: '1.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>
              {trend > 0 ? '↑' : trend < 0 ? '↓' : '—'}
            </div>
      ...
      {!mounted || entries.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14 }}>
          ...
          <Link href="/evaluate" style={{ fontSize: '0.85rem', color: 'var(--accent)', textDecoration: 'none', background: 'var(--accent-dim)', border: '1px solid color-mix(in srgb, var(--accent) 30%, transparent)', padding: '8px 18px', borderRadius: 6 }}>
            Evaluate an Answer →
          </Link>
        </div>
      ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem 1.25rem', cursor: 'pointer' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '0.72rem', fontFamily: 'var(--font-mono)', color: pct >= 70 ? 'var(--green)' : pct >= 50 ? 'var(--warning-text)' : 'var(--danger-text)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); if (confirm('Delete this entry?')) handleDelete(entry.id); }}
                      style={{ background: 'var(--danger-wash)', border: '1px solid color-mix(in srgb, var(--danger-text) 20%, transparent)', color: 'var(--danger-text)', cursor: 'pointer', padding: '3px 8px', borderRadius: 5, fontSize: '0.7rem' }}>
                      ✕
                    </button>
                  </div>
                </div>
                      ...
                      {(['introduction', 'body', 'conclusion', 'presentation'] as const).map(sec => {
                        ...
                        return (
                          <div key={sec} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 6, padding: '0.6rem 0.75rem' }}>
                            ...
                            <div style={{ fontSize: '1rem', fontWeight: 700, fontFamily: 'var(--font-mono)', color: sp >= 70 ? 'var(--green)' : sp >= 50 ? 'var(--warning-text)' : 'var(--danger-text)' }}>
