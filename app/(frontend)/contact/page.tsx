      ...
      <div style={{ width: '100%', maxWidth: '520px' }}>
        ...
        <div style={{ display: 'flex', marginBottom: '32px', borderBottom: '1px solid #1a1a1a' }}>
          {(['contact', 'bug', 'feature'] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setStatus('idle'); }} style={{
              ...
              background: 'none',
              border: 'none',
              borderBottom: tab === t ? '2px solid var(--warning-text)' : '2px solid transparent',
              marginBottom: '-1px',
              fontFamily: 'Inter, system-ui, sans-serif',
              fontSize: '13px',
              fontWeight: tab === t ? 600 : 400,
              color: tab === t ? 'var(--warning-text)' : '#555',
              cursor: 'pointer',
        ...
        {status === 'done' ? (
          ...
          <>
            {tab === 'contact' ? (
              <form onSubmit={handleContact} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <button type="submit" disabled={status === 'sending'} style={{
                  padding: '10px 24px',
                  backgroundColor: status === 'sending' ? 'var(--bg4)' : 'var(--warning-wash)',
                  color: status === 'sending' ? '#555' : 'var(--warning-text)',
            ...
            {status === 'error' && (
              <p style={{ color: 'var(--danger-text)', fontSize: '13px', marginTop: '8px' }}>
