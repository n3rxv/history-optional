...
export const metadata: Metadata = {
  title: 'Early Medieval India PYQs — UPSC History Optional Previous Year Questions',
  description: 'All UPSC History Optional Previous Year Questions on Early Medieval India (Paper I) from 1979–2026.',
  alternates: { canonical: 'https://historyoptional.xyz/pyqs/early-medieval' },
};

export default function PYQPage() {
  const questions = pyqs.filter(q => q.section === 'Paper I - Early Medieval');
  ...
  const schema = {
    ...
    "@graph": [
      {
        "@type": "CollectionPage",
        "@id": "https://historyoptional.xyz/pyqs/early-medieval#page",
        "url": "https://historyoptional.xyz/pyqs/early-medieval",
        "name": "Early Medieval India PYQs — UPSC History Optional",
        "description": questions.length + " Previous Year Questions on Early Medieval India for UPSC History Optional",
      ...
      },
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 3, "name": "Early Medieval India", "item": "https://historyoptional.xyz/pyqs/early-medieval" },
        ]
      }
    ]
  };
    ...
    <>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2.5rem 1.5rem 5rem' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', fontWeight: 700, color: 'var(--text)', marginBottom: '0.5rem' }}>
          Early Medieval India — Previous Year Questions
        </h1>
        <p style={{ color: 'var(--text2)', fontSize: '0.9rem', marginBottom: '2rem' }}>
          UPSC History Optional Paper I · {questions.length} questions · 1979–2026
        </p>
            ...
            <Link key={q.id} href={`/pyqs/${q.id}`} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '0.6rem', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: '0.68rem', fontFamily: 'var(--font-mono)', color: 'var(--accent)', background: 'var(--accent-dim)', border: '1px solid color-mix(in srgb, var(--accent) 20%, transparent)', padding: '2px 8px', borderRadius: 3 }}>{q.topic}</span>
                </div>
