...
export default function Paper2() {
  ...
  const allSlugs = paper2Notes.map(n => n.slug);
  const totalCompleted = mounted ? countCompleted(allSlugs) : 0;
  const total = paper2Notes.length;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem 4rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2.2rem', fontWeight: 700, color: 'var(--text)', marginBottom: '0.5rem' }}>Paper II — Modern India & World History</h1>
        <p style={{ color: 'var(--text2)', fontSize: '0.95rem', maxWidth: 600, marginBottom: '1.5rem' }}>
          {total} topics covering Modern India and World History. Click any topic to open the full notes. Check off topics as you study.
        </p>
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
              <span style={{ color: 'var(--text)', fontSize: '0.85rem', fontWeight: 600 }}>Paper II Progress</span>
            ...
            {mounted && totalCompleted === total && <div style={{ color: 'var(--green)', fontSize: '0.75rem', marginTop: '0.4rem' }}>✓ Paper II complete!</div>}
          </div>
        </div>
      </div>

      {paper2Sections.map(section => {
        const sectionNotes = paper2Notes.filter(n => n.section === section);
                    ...
                    <Link href={`/notes/${note.slug}`} style={{ textDecoration: 'none' }}>
                      <div
                        style={{
                          ...
                          border: `1px solid ${done ? 'color-mix(in srgb, var(--success-text) 25%, transparent)' : 'var(--border)'}`,
                          borderRadius: 8, padding: '1.1rem 2.5rem 1.1rem 1.25rem',
                          transition: 'all 0.15s', cursor: 'pointer',
                          display: 'flex', gap: '1rem', alignItems: 'flex-start',
                        }}
                        onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = done ? 'color-mix(in srgb, var(--success-text) 50%, transparent)' : 'var(--accent2)'; el.style.background = 'var(--bg3)'; }}
                        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = done ? 'color-mix(in srgb, var(--success-text) 25%, transparent)' : 'var(--border)'; el.style.background = done ? 'var(--bg3)' : 'var(--bg2)'; }}
                      >
                        <span style={{
                          minWidth: 28, height: 28,
                          background: done ? 'var(--green-dim)' : 'var(--accent-dim)',
                          border: `1px solid ${done ? 'color-mix(in srgb, var(--success-text) 40%, transparent)' : 'var(--accent2)'}`,
