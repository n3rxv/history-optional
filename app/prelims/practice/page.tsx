    ...
    <div style={{ minHeight: '100vh', background: '#080810', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', fontSize: '0.82rem' }}>
          ...
          {answered > 0 && (
            <button onClick={() => { if (confirm('Reset all attempts? Bookmarks will be kept.')) { setStates(prev => { const next: Record<string, any> = {}; Object.entries(prev).forEach(([id, s]) => { if ((s as any).marked) next[id] = { selected: null, submitted: false, marked: true, aiResult: null, aiLoading: false }; }); try { localStorage.setItem('ho_prelims_states', JSON.stringify(next)); } catch {} return next; }); setCurrent(0); } }}
