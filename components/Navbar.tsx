    ...
    <nav style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }} className="desktop-nav">
          ...
          {/* ── PRELIMS link (new) ── */}
          <Link href="/prelims" style={{
            padding: '0.3rem 0.65rem', borderRadius: 6,
            fontSize: '0.82rem', fontFamily: 'var(--font-ui)', textDecoration: 'none',
            display: 'flex', alignItems: 'center', gap: '0.3rem',
            color: pathname.startsWith('/prelims') ? '#000' : 'rgba(255,255,255,0.75)',
            background: pathname.startsWith('/prelims') ? 'var(--accent)' : 'rgba(59,130,246,0.08)',
            border: `1px solid ${pathname.startsWith('/prelims') ? 'transparent' : 'rgba(59,130,246,0.2)'}`,
            transition: 'all 0.15s', fontWeight: pathname.startsWith('/prelims') ? 700 : 500,
          }}
          onMouseEnter={e => { if (!pathname.startsWith('/prelims')) { (e.currentTarget as HTMLElement).style.background = 'rgba(59,130,246,0.15)'; (e.currentTarget as HTMLElement).style.color = '#fff'; } }}
          onMouseLeave={e => { if (!pathname.startsWith('/prelims')) { (e.currentTarget as HTMLElement).style.background = 'rgba(59,130,246,0.08)'; (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.75)'; } }}
          >
      ...
      {open && (
        <div style={{
          ...
          {[
            ...
            { href: '/test', label: 'Start Test' },
            { href: '/prelims', label: '⏱ Prelims MCQs' },
            { href: '/chat', label: 'AI Chat' },
            { href: '/evaluate', label: 'Evaluate' },
