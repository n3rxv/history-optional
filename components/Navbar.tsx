      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1100, background: 'rgba(0,0,0,0.92)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 72 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }} className="desktop-nav">
            ...
            {/* Notes dropdown */}
            <div ref={notesRef} style={{ position: 'relative' }} onMouseEnter={() => { if (!isTouch.current) setNotesMenuOpen(true); }} onMouseLeave={() => { if (!isTouch.current) setNotesMenuOpen(false); }}>
            ...
            {/* PYQs dropdown */}
            <div ref={pyqsRef} style={{ position: 'relative' }} onMouseEnter={() => { if (!isTouch.current) setPyqsMenuOpen(true); }} onMouseLeave={() => { if (!isTouch.current) setPyqsMenuOpen(false); }}>
