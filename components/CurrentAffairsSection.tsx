...
function FeaturedCard({ post, onClick, authed, index }: { post: Post; onClick: () => void; authed: boolean; index: number }) {
  ...
  return (
    <article onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ cursor: 'pointer', display: 'grid', gridTemplateColumns: post.cover_image ? '48px 80px 1fr' : '48px 1fr', gap: '0 1.5rem', padding: '1.5rem 0', borderBottom: '1px solid var(--border)', position: 'relative', transition: 'opacity 0.15s', opacity: hovered ? 0.85 : 1 }}>
      ...
      {post.cover_image && (
        <div style={{ width: 80, height: 80, borderRadius: 6, overflow: 'hidden', alignSelf: 'center', flexShrink: 0 }}>
