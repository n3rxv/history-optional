...
# Fix TelegramFloat.tsx — correct backtick template literal (no escaping)
cat > components/TelegramFloat.tsx << 'EOF'
...
echo "✓ components/TelegramFloat.tsx written"

git add components/TelegramFloat.tsx
git commit -m "fix: correct template literal in TelegramFloat (build error)"
git push
