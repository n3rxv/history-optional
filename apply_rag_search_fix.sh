#!/bin/bash
# Apply rag-search/route.ts fix — replace Jina with local embed service

TARGET="app/api/rag-search/route.ts"

echo "Checking file exists..."
[ -f "$TARGET" ] || { echo "ERROR: $TARGET not found. Run from project root."; exit 1; }
...
echo "Verifying..."
grep -q "EMBED_SERVICE_URL" "$TARGET" && echo "✅ EMBED_SERVICE_URL present" || echo "❌ EMBED_SERVICE_URL missing"
grep -q "localEmbed" "$TARGET" && echo "✅ localEmbed present" || echo "❌ localEmbed missing"
grep -q "jina" "$TARGET" && echo "❌ Jina still present!" || echo "✅ Jina removed"
